...
<body class="flex min-h-screen flex-col overflow-hidden bg-white">
    <nav class="flex items-center justify-between px-6 py-3 shadow-sm">
        <div class="flex items-center space-x-2 overflow-hidden">
            ...
            <h1 class="text-lg font-bold text-[#E60023]">Pinterest</h1>
            <a href="#" class="ml-4 font-semibold text-black hover:underline">Explore</a>
        </div>

        <div class="flex items-center space-x-6 font-medium text-blue-700">
        ...
        <div class="flex items-center space-x-3">
            @auth
                <a href="{{ route('inicioLogueado') }}"
                    class="inline-block rounded-full bg-[#E60023] px-4 py-2 text-center font-semibold text-white transition hover:bg-[#ad0019]">
                    Ver Feed
                </a>
                <form method="POST" action="{{ route('logout') }}" class="inline">
                    @csrf
                    <button type="submit"
                        class="rounded-full bg-gray-100 px-4 py-2 font-semibold text-black transition hover:bg-gray-200">
                        Cerrar sesión
                    </button>
                </form>
            @else
                ...
                <a href="{{ route('registro') }}"
                    class="inline-block rounded-full bg-gray-100 px-4 py-2 text-center font-semibold text-black transition hover:bg-gray-200">
                    Registrarse
                </a>
            @endauth
        </div>
    ...
    <main id="scrollContainer" class="h-screen flex-grow snap-y snap-mandatory overflow-x-hidden overflow-y-scroll">

        <section class="h-screen snap-start overflow-hidden">
    ...
    <footer class="w-full border-t border-gray-200 bg-white py-4">
        <div class="mx-auto flex max-w-6xl flex-wrap justify-center space-x-4 px-4 text-sm text-gray-600">
            <a href="{{ route('Condiciones') }}" class="text-blue-600 hover:underline">Condiciones de servicio</a>
            <a href="{{ route('PoliticasPrivacidad') }}" class="text-blue-600 hover:underline">Política de
                privacidad</a>
